...
/// A compact opening book that maps FEN piece-placement strings to opening names.
///
/// The FEN keys use only the first field (piece placement) so that the book
/// remains independent of side-to-move, castling rights, and en passant –
/// this matches the format used in the bundled `openings.json`.
pub struct OpeningBook {
    table: HashMap<String, String>,
}

impl OpeningBook {
    /// Creates an opening book from a JSON string.
    ///
    /// The expected format is a flat object whose keys are FEN piece-placement
    /// strings and whose values are human-readable opening names:
    ///
    /// ```json
    /// {
    ...
    /// Creates an empty opening book (used when no `openings.json` is provided).
    pub fn empty() -> Self {
        Self {
            table: HashMap::new(),
        }
    ...
    /// Looks up the opening name for a given FEN piece-placement string.
    ///
    /// Returns `None` if the position is not in the book.
    pub fn lookup(&self, placement: &str) -> Option<&str> {
        self.table.get(placement).map(String::as_str)
    }

    /// Returns the total number of positions in the book.
...
/// Minimal set of well-known openings bundled directly into the library binary.
///
/// This constant is used as a fallback when no external `openings.json` is
/// loaded. It covers the most common first moves and a selection of popular
/// opening systems so that users of the library get useful output even without
/// a separate data file.
pub const BUILTIN_OPENINGS_JSON: &str = r#"
{
